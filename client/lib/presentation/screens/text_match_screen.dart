...
import '../widgets/dev_tools_panel.dart' show GamePokeFeedback;
        ...
        child:
          ...
          KoPanel(
              child: Column(
                ...
                Text(s.json['contract']['eligibility']['rewards']
                    ? l.textRewardsOn
                    : l.textRewardsOff),
                Text(switch (s.mode) {
            ...
            _section(l.textResults, [
              for (final n in s.json['verdict_nowns'] ?? [])
                Padding(
                    ...
                    child: KoPanel(child: _authored(n['content']['text'])))
            ]),
      ...
      case 'make_room':
        ...
        items = [
          for (final c in slots)
            _card(c['card'],
                caption:
                    '${l.textSlot} ${c['slot'] + 1} · ${_actor(c['actor'])}')
        ];
        break;
      case 'bad_bargains':
        items = [
              ...
              child: Directionality(
                  ...
                  child: Semantics(
                      ...
                      child: KoButton(
                          ...
                          label: card.text,
                          color: _copy == card.copyID
                              ? KoColors.lime
                              : KoColors.surface,
                          onPressed: enabled &&
                                  s.capabilities.any(textModeActions.contains)
  ...
  Widget _preview() {
    ...
    return _section(l.textPreview, [
      ...
      if (s.mode == 'bad_bargains') ...[
        Text(l.textTarget),
        for (final c in board.where((c) =>
            c['seat'] != s.seat &&
            (s.json['seats'] as List).any((p) =>
                p['seat'] == c['seat'] && p['connected'] && !p['eliminated'])))
          KoButton(
  ...
  bool _pokeAllowed(dynamic p) {
    ...
    String group(String phase) => ['play', 'trade_response'].contains(phase)
        ...
        : phase == 'discussion'
            ? 'discussion'
            : 'voting';
    if ((s.json['history'] as List).any((e) =>
        e['kind'] == 'poke' &&
        e['round'] == s.round &&
        e['actor']['seat'] == s.seat &&
            ...
            child: KoPanel(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      if (e['target_seat'] != null)
                        Text(_seat(e['target_seat'])),
                      if (e['text'] != null) _authored(e['text']),
                      if (e['phrase_id'] != null)
                        Text(e['phrase_id'] == 'chat.hidden'
                            ? l.textChatHidden
                            : quickChatPhraseLabel(l, e['phrase_id'])),
                      for (final card in e['cards'])
                        _authored(card['content']['text']),
                      if (e['ballot'] != null)
                        Text(e['ballot']['result']['outcome'] == 'runoff'
                            ? l.textRunoff
                      ...
                      if (e['ballot'] != null)
                        for (final v in e['ballot']['votes'])
                          Text(
                              '${_seat(v['seat'])} → ${_seat(v['target_seat'])}')
                    ]))),
      if (pages > 1)
...
class _TextFallingResult extends StatelessWidget {
  const _TextFallingResult(
      {required this.remaining, required this.label, super.key});
    ...
    return ClipRect(
        child: TweenAnimationBuilder<double>(
            ...
            builder: (_, value, child) => Transform.translate(
                ...
                child: Opacity(opacity: 1 - value * .65, child: child)),
            child: panel));
  }
}

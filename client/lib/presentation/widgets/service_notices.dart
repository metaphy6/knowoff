...
class ServiceNoticeBanner extends StatefulWidget {
  const ServiceNoticeBanner({this.api, super.key});
  final ApiClient? api;
  @override
...
class _ServiceNoticeBannerState extends State<ServiceNoticeBanner> {
  late final Future<List<dynamic>> _notices =
      serviceApi(widget.api).getNotices();
  ...
  Widget build(BuildContext context) => FutureBuilder<List<dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final notices = snapshot.data!
            ...
            .where((notice) =>
                !_dismissed.contains('${notice['id'] ?? notice['title']}'))
            .toList();
        return Column(
...
class ServiceNoticeTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final start = startValue is num
        ? DateTime.fromMillisecondsSinceEpoch(startValue.toInt() * 1000,
            isUtc: true)
    ...
    return KoPanel(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(serviceNoticeText(notice['title'], locale),
...
class _MaintenanceClockState extends State<_MaintenanceClock> {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final seconds =
        widget.start.difference(DateTime.now()).inSeconds.clamp(0, 99999999);
    final duration =

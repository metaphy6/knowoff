...
void main() {
  testWidgets('compact header gives unused action width back to its title',
    ...
    expect(
        tester.renderObject<RenderParagraph>(title).didExceedMaxLines, isFalse);
  ...
  testWidgets(
    ...
    await tester.pumpWidget(MaterialApp(
        home:
            Scaffold(body: KoButton(label: 'Play', onPressed: () => taps++))));
  ...
  test('all house shadows remain hard and palette stays legible', () {
    ...
    for (final fill in [
      ...
      final contrast = (fill.computeLuminance() + 0.05) /

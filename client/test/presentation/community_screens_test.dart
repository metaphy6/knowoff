...
void main() {
  for (final tied in [false, true]) {
    testWidgets('closed challenge shows the server winner when tied=$tied', (
      ...
      expect(
        find.text('The week is closed. Meet your Week Winner.'),
        findsOneWidget,
      );
  ...
  testWidgets('forbidden challenge read explains account availability', (
    ...
    await _pump(tester, WeeklyChallengeScreen(api: api));
    expect(
      find.text('This account cannot participate in the challenge right now.'),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(
        find.byKey(const Key('challenge-content')),
        'My careful draft',
      );
  ...
  testWidgets(
    'pairing requires a valid code and explicit browser confirmation',
    (tester) async {
      final api = _Api();
      Uri? opened;
      await _pump(
  ...
  testWidgets(
    'one immutable vote needs confirmation and disables later votes',
    (tester) async {
      final api = _Api();
      await _pump(tester, WeeklyChallengeScreen(api: api));
  ...
  for (final size in [
    ...
    testWidgets(
      'community screens fit $size with large pseudo-localized text',
      (tester) async {
        final api = _Api();
        await _pump(
          tester,
          WeeklyChallengeScreen(api: api),
          size: size,
          scale: 2,
        ...
        await _tap(tester, 'challenge-refresh');
        expect(
          find.byKey(const Key('challenge-winner-entry-2')),
          findsOneWidget,
        );
        expect(tester.takeException(), isNull);
        await _pump(
          tester,
          ContributorConnectScreen(api: api),
          size: size,
          scale: 2,
          locale: const Locale('en', 'XA'),
        );

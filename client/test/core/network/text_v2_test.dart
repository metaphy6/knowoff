...
void main() {
  test(
    ...
    for (final key in ['points', 'xp', 'leaderboard_counted', 'interrupted']) {
      ...
      changed['settlement'][key] = key == 'leaderboard_counted'
          ...
          : key == 'interrupted'
              ? false
              : 1;
      expect(() => V2Codec.control('delivery', changed),
  ...
  test('eliminated snapshots cannot retain another private hand', () {
    final fixtures = (jsonDecode(
        File('../server/internal/transport/v2/testdata/contracts.json')
            .readAsStringSync()) as List);
    final hidden =
        fixtures.singleWhere((f) => f['name'] == 'snapshot-eliminated')['wire'];
    hidden['private']['hand'] =
        fixtures.singleWhere((f) => f['name'] == 'snapshot-nower')['wire']
            ['private']['hand'];
    ...
    expect(() => V2Snapshot.decode(jsonEncode(hidden), bounds),
        throwsA(isA<V2Failure>()));
  });
  ...
  test(
    ...
    for (final f in fixtures) {
      ...
      if (f['code'] == '') {
        expect(() => V2Codec.decode(f['kind'] as String, raw, limits),
            returnsNormally,
  ...
  test(
    ...
    for (final raw in [
      ...
      expect(() => V2Codec.decode('action', raw, limits),
          throwsA(isA<V2Failure>()));
    }
    expect(
        () => V2Codec.decode('action', ' ' * 65537, limits),
        throwsA(isA<V2Failure>()
            .having((e) => e.code, 'code', 'protocol.frame_too_large')));
  });
  test('decoded snapshots detach and freeze all private and public collections',
      () {
    final wire =
        fixtures.singleWhere((f) => f['name'] == 'snapshot-nower')['wire'];
    ...
    expect(() => s.json['contract']['mode_id'] = 'top_that',

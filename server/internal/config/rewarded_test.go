...
func TestRewardedConfigurationClosedAndBounded(t *testing.T) {
	...
	base := RewardedConfig{Enabled: true, MaxQueryBytes: 16384, MaxResponseBytes: 262144, HTTPTimeoutS: 10, KeyCacheS: 3600, KeyRefreshMinS: 60, MaxConcurrentRequests: 4, ClaimTTLS: 1800, AdUnits: map[string]RewardedUnit{"123": {RewardItem: "match_bonus", RewardAmount: 1}}}
	if err := base.Validate(); err != nil {
		t.Fatal(err)
	}
	for _, bad := range []func(*RewardedConfig){
		func(c *RewardedConfig) { c.MaxQueryBytes = 0 }, func(c *RewardedConfig) { c.MaxResponseBytes = 1048577 }, func(c *RewardedConfig) { c.HTTPTimeoutS = 0 }, func(c *RewardedConfig) { c.KeyCacheS = 86401 }, func(c *RewardedConfig) { c.KeyRefreshMinS = 0 }, func(c *RewardedConfig) { c.MaxConcurrentRequests = 33 }, func(c *RewardedConfig) { c.ClaimTTLS = 0 }, func(c *RewardedConfig) { c.AdUnits = nil }, func(c *RewardedConfig) {

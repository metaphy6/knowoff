...
const overviewBody = `<div class="split"><section><div class="panel accent"><h2 style="margin-top:0">Bring your wonderfully specific brain.</h2><p>Write something worth playing. Keep it surprising, readable, and yours.</p><a class="button" href="/portal/submissions">Open my submissions</a></div><h2>Your studio</h2><p>Active role: <span class="status">{{if .Data.Role}}{{.Data.Role}}{{else}}Player{{end}}</span></p><p>{{.Data.Submissions}} saved submissions. Approved work earns contributor credits and Noin through the server reward rules.</p><a href="/portal/apply">View applications and permissions</a>{{if eq .Data.Role "guard"}}<p><a href="/portal/guard">Open Guard review</a></p>{{end}}</section><aside><h2 style="margin-top:0">From thought to table</h2><ol class="list"><li>Accept the current contribution terms and save a text draft.</li><li>Submit when it is ready. It becomes read-only in the queue.</li><li>Automated screening and a human decision come before publication.</li></ol><p class="muted">Text contributions are open here. Approved text enters curation; a reviewed release is required before it becomes playable.</p><a href="/portal/challenge">See this week's community challenge</a></aside></div>`
const applicationBody = `<div class="split"><section><h2 style="margin-top:0">Choose how you contribute</h2><p>Your level: <strong>{{.Data.Level}}</strong>. Applications open at level <strong>{{.Data.MinLevel}}</strong>.</p><p>Current role: <span class="status">{{if .Data.Role}}{{.Data.Role}}{{else}}Player{{end}}</span></p><form method="post" action="/portal/apply"><input type="hidden" name="csrf_token" value="{{.CSRF}}"><label for="role">Role</label><select id="role" name="role"><option value="contributor">Contributor — submit content</option><option value="curator">Curator — content review and authoring</option><option value="guard">Guard — community safety</option></select><p class="small muted">Contributor and Curator permissions cover content work. Guard is a separate community-safety role.</p><button {{if not .Data.Eligible}}disabled{{end}}>Send application</button></form></section><section><h2 style="margin-top:0">Your applications</h2>{{range .Data.Applications}}<article class="panel"><div class="row spread"><h3>{{.Role}}</h3><span class="status">{{.Status}}</span></div><p class="small">Applied {{.AppliedAt.Format "02 Jan 2006"}}</p>{{if .Reason}}<p>{{.Reason}}</p>{{end}}</article>{{else}}<p class="empty">No application yet. Choose a role when your account is eligible.</p>{{end}}</section></div>`

...
type CutoverRoleSpec struct{ Database, Owner, Runtime, Capture, Migrator string }
...
var cutoverTables = strings.Fields("accounts admin_accounts admin_audit_log admin_operation_decisions admin_operation_results admin_sessions audit_events auth_revocations billing_account_sources billing_legacy_premium billing_provider_tasks billing_subscription_current billing_subscription_imports billing_subscription_observations billing_subscription_replacements billing_subscription_sources billing_subscription_tasks billing_transactions challenge_current_winner challenge_entries challenge_topics challenge_votes challenge_winners custom_avatars cutover_handoffs cutover_instances cutover_requests cutover_watermarks daily_noin_earned daily_quickplay_counts device_tokens entitlements feedback guard_freezes leaderboard_daily_counts leaderboard_entries leaderboard_history leaderboard_weeks named_entitlement_items noin_ledger noin_wallets oauth_flows oauth_links player_blocks portal_browser_sessions portal_login_limits portal_login_requests portal_role_applications portal_roles portal_submission_counts portal_submissions portal_terms profiles queue_cooldowns report_cases report_rate_limits reports schema_migrations store_purchases system_notices text_abandons text_accepted_inputs text_active_releases text_admissions text_archive_progress text_award_receipts text_content_revisions text_first_win_claims text_legacy_archive text_matches text_outbox text_process_current text_process_owners text_releases text_settlements user_terms_acceptances user_terms_versions")
...
var cutoverInsertOnlyTables = []string{"admin_audit_log", "admin_operation_decisions", "admin_operation_results"}
...
var cutoverFunctions = strings.Fields("billing_retained_identity() billing_subscription_immutable() billing_subscription_projection_guard() billing_subscription_receipt_guard() billing_subscription_replacement_guard() billing_subscription_task_guard() cutover_guard_handoff() cutover_guard_instance() cutover_guard_request() cutover_guard_watermark() cutover_require_bound_request() keep_avatar_revision_monotonic() keep_session_epoch_monotonic() protect_account_auth_purpose() refuse_retained_value_truncate() report_protect_target() retain_text_abandon() text_freeze_archive_sources() text_protect_applied_effects() text_protect_closed_ranking() text_protect_delivery_identity() text_protect_match_identity() text_protect_process_binding() text_protect_process_owner() text_protect_rank_snapshot() text_protect_release_identity() text_protect_reviewed_source() text_protect_week_identity() text_refuse_value_rewrite()")
...
SELECT (SELECT count(*)=cardinality($1::text[]) AND bool_and(NOT rolsuper AND NOT rolcreatedb AND rolcreaterole=(rolname=$7) AND NOT rolreplication AND NOT rolbypassrls AND NOT rolinherit AND rolcanlogin=(rolname<>$2 AND (rolname NOT IN($3,$5) OR $8::boolean))) FROM roles)
AND (SELECT count(*)=CASE WHEN $7='' THEN 1 ELSE 5 END FROM pg_auth_members WHERE member IN(SELECT oid FROM roles) OR roleid IN(SELECT oid FROM roles))
...
readers AS (SELECT oid FROM pg_roles WHERE rolname IN($2,$3,$4)),
...
actors AS (SELECT oid FROM pg_roles WHERE rolname IN($1,$2,$3,$7)),
...
AND NOT EXISTS(SELECT 1 FROM pg_attribute a JOIN objects c ON c.oid=a.attrelid WHERE a.attacl IS NOT NULL)
...
AND NOT EXISTS(SELECT 1 FROM functions WHERE proowner<>(SELECT oid FROM own) OR prosecdef OR prorettype<>'trigger'::regtype OR prokind<>'f')

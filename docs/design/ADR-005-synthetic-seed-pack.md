...
GPU/content pipeline produces final media. The pack must be deterministic,
lightweight, copyleft-clear, and regenerate in CI without API keys or GPUs.
...
- Real image/GIF packs will replace or supplement this pack once the content
  pipeline produces certified bundles; the format and loader are the same.

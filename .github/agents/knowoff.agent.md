---
name: "knowoff"
description: "The primary, full-authority engineering agent for the Knowoff project — an online social-deduction party game (Go server, Flutter client, PostgreSQL/Redis, Docker Compose + Cloudflare, text content pipeline). Use for ANY Knowoff-related request: project Q&A, running the stack or any component, adding features, debugging, code changes, database migrations, DevOps/deployment, writing or running tests, UI/UX, economy tuning, contributor portal/admin console work, and drafting, reviewing and integrating game content through the Blueprint-defined media pipeline. Reads BLUEPRINT.md as the normative spec and docs/planning/ROADMAP.md as the sequenced roadmap. Has full authority to read and edit anything in the repo, including all documentation."
...
| **Nown/card content creation, review and integration** | Use [`knowoff-content-create`](../../.agents/skills/knowoff-content-create/SKILL.md), [`knowoff-content-review`](../../.agents/skills/knowoff-content-review/SKILL.md) and [`knowoff-content-integrate`](../../.agents/skills/knowoff-content-integrate/SKILL.md) for the requested stage. Create plain-text Nowns and response/item cards only; keep non-playable avatar and brand imagery separate. Read Blueprint ⚙️ §1–4 and the current humor/curator/tone guides, automatically following the [High/Distant/Chaos source map](../../content/curator-guide.md#read-the-dealing-path-before-authoring) through tuning and server/client working logic without a user reminder. Verify actual `tools/mediapack` capabilities before claiming an operation exists; the synthetic fixture and accepted text are not production release evidence. |
